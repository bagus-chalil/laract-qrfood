...
import ModalEditDataUsers from './ModalEditDataUsers';
...
export default function TableUsers({ dataUser }) {
    ...
    const filter = useRef(dataUser.per_page);
    const { url } = usePage();
    ...
    const editDataUser = (dataUser) => {
        setSelectedDataUser(dataUser);

        setData({
            name: dataUser.name,
            email: dataUser.email,
            no_telephone: dataUser.no_telephone
        });
    ...
    const submitEdit = (e) => {
        ...
        post(`/users/update/${selectedDataUser.id}`, {
            onSuccess: () => {
    ...
    return (
        <>
            ...
            <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        ...
                        <th scope="col" className="px-6 py-3">No Telephone</th>
                        <th scope="col" className="px-6 py-3">Kode Referal</th>
                        <th scope="col" className="px-6 py-3">Blas Email</th>
                    ...
                    {isLoading ? (
                        ...
                        dataUser.data.map((item, index) => (
                            <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600" key={item.id}>
                                <td className="px-6 py-4">{dataUser.from + index}</td>
                                <th scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {item.name}
                                </th>
                                <td className="px-6 py-4">{item.email}</td>
                                <td className="px-6 py-4">{item.no_telephone}</td>
                                <td className="px-6 py-4">{item.referal_code}</td>
                                <td className="px-6 py-4">{item.send_email == 1 ? 'Sudah Blas' : 'Belum Blas'}</td>
                                <td className="px-6 py-4 text-left rtl:text-center">
                                    <div className="flex items-center gap-2 rounded">
                                        <button onClick={() => editDataUser(item)} className="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</button>
                                    </div>
                                </td>
                            </tr>
                        ))
            ...
            <div className="flex flex-col sm:flex-row flex-wrap space-y-4 sm:space-y-0 items-center justify-between pb-4 m-4">
                <div>Showing {dataUser.from} to {dataUser.to} total {dataUser.total}</div>
